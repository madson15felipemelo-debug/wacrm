...
export function AiUsageCard() {
  ...
  const fetchUsage = useCallback(async (windowDays: number) => {
    ...
    try {
      ...
      toast.error('Falha ao carregar o consumo');
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  const chartData =
    data?.daily.map((d) => ({ day: format(parseISO(d.date), 'd MMM', { locale: dateLocale }), Tokens: d.tokens })) ??
    [];
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>
              Tokens gastos na sua chave do provedor pelos rascunhos e pelo bot
              de resposta automática. Só a contagem — nenhum conteúdo de mensagem
              é guardado aqui.
            </CardDescription>
          </div>
          <Select
      ...
      <CardContent className="space-y-5">
        {loading || !data ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center text-sm text-muted-foreground">
            ...
            <p className="text-xs">
              Isso se preenche conforme o assistente redige e responde.
            </p>
          </div>
        ) : (
          <>
            ...
            {data.truncated && (
              <p className="text-xs text-muted-foreground">
                Exibindo uma janela parcial — o uso é alto o bastante para que
